...
/// The running-status writer, held to the same shared contract as the naive
/// writer (`SMFWriterSpecs`, PLAN.md §0.5). Both implementations passing one
/// suite is the proof that makes the swap a deletion, not a rewrite.
@Suite("Running-status SMF writer meets the writer contract")
struct RunningStatusSMFWriterSpecTests: SMFWriterSpecs {
    ...
    func makeSUT() -> any SMFWriting { RunningStatusSMFWriter() }
}
